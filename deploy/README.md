...
## Модель семантического поиска (необязательно, но желательно)

Без неё память ищет по буквам, и «кожа» оказывается релевантнее «кота».
Бот при этом работает — поиск молча деградирует до строкового.

```bash
cd /var/www/mindnumbness
./venv/bin/pip install -r requirements.txt

curl -L -o storage/models/navec_hudlit_v1_12B_500K_300d_100q.tar \
...
Стоимость: 51 МБ на диске, **~390 МБ RAM на процесс**, поиск по 150 узлам
5 мс. На машине с 3.7 ГБ и тремя соседними сервисами это заметно —
если памяти станет мало, отключается без правки кода:
...
Векторы досчитываются лениво при первом обращении к узлу, отдельной
миграции базы не нужно.

---

# Mini App «Карта памяти»
