...
# Поля, чьи умолчания ОБЯЗАНЫ расходиться с config.py.
#
# db_path — там абсолютный путь этой установки, а библиотека не вправе
# писать в чужой каталог.
#
# default_user_model — это текст ПЕРСОНАЖА, а не параметр памяти. В
# config.py он описывает наставника цифрового ребёнка по-русски; в
# библиотеке стоит нейтральная английская заглушка, потому что
# осмысленное значение всегда приходит от приложения.
#
# Совпадение значений в обоих случаях было бы ошибкой, а не порядком.
# Пути и тексты — настройки РАЗВЁРТЫВАНИЯ, а не параметры поведения.
# embedding_model_path попал сюда после неловкой находки: в библиотеке
# он был захардкожен абсолютным путём на машину разработчика, поэтому
# семантика не работала ни у кого, кроме автора, а часть тестов проходила
# только у него. У витрины путь свой и законный; у библиотеки его быть не
# должно вовсе.
NOT_CALIBRATED = {"db_path", "default_user_model", "embedding_model_path"}
