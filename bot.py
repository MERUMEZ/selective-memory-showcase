...
# растёт от ВСПОМИНАНИЯ, не от записи.
#
# ЧЕСТНО О ТОМ, ЧТО ПОКАЖЕТ ЭТОТ СЦЕНАРИЙ. Замер на 15 сидах, горизонт
# 7 суток: телефон переживает первым в 9 прогонах, рутина в 1, ещё в 5
# не выживает ничего. То есть пятнадцати реплик мало, чтобы эффект
# гарантировался, — забывание часто уносит всё. Но когда что-то
# переживает, это почти всегда подкреплённое.
#
# Показывать это надо именно так, а не обещать неизбежный результат:
# у зрителя прогон один, и он вполне может попасть в те пять.
# Сильное число (+53 п.п.) живёт в tools/compare_retention.py, где
# разговор в разы длиннее.
DEMO_SCRIPT = [
    "привет как дела",
    "меня зовут Паша",
    "привет как дела",
